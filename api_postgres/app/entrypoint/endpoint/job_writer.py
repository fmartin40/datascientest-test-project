...
async def create_salaire(
    ...
    try:
        result = await job_writer.add_salaire(salaire.libelle)
        return {"status": "success", "id": result.id, "libelle": result.libelle}
    except IntegrityError as e:
        logger.warning(f"Erreur d'intégrité ignorée: {str(e)}")
        if "duplicate key value violates unique constraint" in str(e):
            return {"status": "success", "message": "Le salaire existe déjà, aucune modification apportée"}
        return {"status": "warning", "message": f"Problème d'intégrité des données: {str(e)}", "error": str(e)}
    except Exception as e:
        logger.error(f"Erreur lors de la création du salaire: {str(e)}")
        return {"status": "error", "message": f"Une erreur est survenue: {str(e)}", "error": str(e)}
...
async def create_formation(
    ...
    try:
        result = await job_writer.add_formation(formation.libelle)
        return {"status": "success", "id": result.id, "libelle": result.libelle}
    except IntegrityError as e:
        logger.warning(f"Erreur d'intégrité ignorée: {str(e)}")
        if "duplicate key value violates unique constraint" in str(e):
            return {"status": "success", "message": "La formation existe déjà, aucune modification apportée"}
        return {"status": "warning", "message": f"Problème d'intégrité des données: {str(e)}", "error": str(e)}
    except Exception as e:
...
async def create_experience(
    ...
    try:
        result = await job_writer.add_experience(experience.libelle)
        return {"status": "success", "id": result.id, "libelle": result.libelle}
    except IntegrityError as e:
        logger.warning(f"Erreur d'intégrité ignorée: {str(e)}")
        if "duplicate key value violates unique constraint" in str(e):
            return {"status": "success", "message": "L'expérience existe déjà, aucune modification apportée"}
        return {"status": "warning", "message": f"Problème d'intégrité des données: {str(e)}", "error": str(e)}
    except Exception as e:
        logger.error(f"Erreur lors de la création de l'expérience: {str(e)}")
        return {"status": "error", "message": f"Une erreur est survenue: {str(e)}", "error": str(e)}
