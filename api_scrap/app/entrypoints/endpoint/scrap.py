from app.domain.profile.dtos.scrapjob import (
    ScrapJobSummaryInputDto,
...
async def list_jobs_summaries(
    ...
    try:
        task_name = (
            "dynamique.extract_summaries" if dynamique else "statique.extract_summaries"
        )
        task = celery_client.send_task(
            task_name, kwargs=input_dto.model_dump()  # Nom exact de la tâche
        )
...
async def get_job_detail(
    ...
    try:
        task_name = (
            "dynamique.extract_jobdetail" if dynamique else "statique.extract_jobdetail"
        )
        task = celery_client.send_task(
