...
class RedisLoader(ILoader):
    ...
    def insert(self, key: str, job:Dict|List[Dict], ttl:int):
        try:    
    ...
    def _insert_detail(self, key: str, job:Dict, ttl:int):
        try:
    ...
    def _insert_summaries(self, key: str, job:List[Dict], ttl:int):
        try:
            self.redis.set(key, json.dumps(job), ex=ttl) # type: ignore
        except Exception as e:
