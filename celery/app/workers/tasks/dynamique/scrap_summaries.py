...
import logging
from app.workers.tasks.dynamique.scrap_detail import extract_jobdetail

logger = logging.getLogger(__name__)


@shared_task(
    name="dynamique.extract_summaries",
...
def extract_summaries(
    ...
    try:
        logger.info(
            f"[{website}] Début de l'extraction - query: '{query}', location: '{location}', pages: {loop}"
        )
        ...
        for i, summary in enumerate(valid_summaries[:5]):
            try:
                ...
                logger.error(
                    f"[{website}] Échec de lancement pour URL: {summary.url} - {task_error}",
                    exc_info=True,
                )
