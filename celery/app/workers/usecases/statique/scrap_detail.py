...
def extract_jobdetail(summary: dict, source: str):
    try:
        ...
        job_detail = asyncio.run(transform_job_detail(job_detail=job_detail))
        ...
        # postgres_loader.insert(key=task_id, job=[job_detail.model_dump()], ttl=300)
...
async def transform_job_detail(job_detail: JobDetail):
    try:
        competences: List[str] = await container.keyword_transformer(keywords=COMPETENCES).transform(text=job_detail.description, type="liste") # type: ignore
        mode_travail: str = await container.keyword_transformer(keywords=MODE_TRAVAIL).transform(text=job_detail.description, type="keyword") # type: ignore
        type_contrat: str = await container.keyword_transformer(keywords=TYPE_CONTRAT).transform(text=job_detail.description, type="keyword") # type: ignore
        duree_travail: str = await container.keyword_transformer(keywords=DUREE_TRAVAIL).transform(text=job_detail.description, type="keyword") # type: ignore
