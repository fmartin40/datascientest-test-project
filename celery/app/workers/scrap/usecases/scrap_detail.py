...
def extract_jobdetail_for_test(url: str, website: str):
    try:
        ...
        # asyncio.run(redis_detail.insert(key=task_id, job=job))
        # asyncio.run(structured_db.insert(job=job_transformed))
        # asyncio.run(unstructured_db.insert(job=job_transformed))
